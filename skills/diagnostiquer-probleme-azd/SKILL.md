...
Reproduire, isoler, prouver, puis s'arrêter au diagnostic.
...
1. Reproduire (`reproduce`) l'échec avec le plus petit cas utile: commande, seed/env, input, expected, actual, timestamp et artifact.
2. Tenir un `diagnosis_ledger` reproductible où chaque tentative garde command, exit/status, output digest et lien artifact.
...
Le skill rend `diagnosis` ([diagnosis-output.md](references/diagnosis-output.md)) avec les champs author_id, reviewer_id, reproduction, diagnosis_ledger, hypothesis_ledger, author_evidence, reviewer_evidence, root_cause, invalidated_assumption, graph_impact, confidence, evidence, verdict.
