...
14. Pour tout claim qui porte sur l'application réelle, appeler `$verifier-application-azd executer` (lancer l'application en isolation, exercer la fonctionnalité, capturer l'artefact) ; il met à jour la feature map tenue par `verifier-application-azd`, qui documente la dernière preuve, jamais un substitut à une preuve fraîche. Le protocole de preuve visuelle annotée vit dans `skills/verifier-application-azd/references/preuve-visuelle.md` (`manifest.json`, `report.md`) ; ce skill consomme l'artefact et le statut qu'il produit, jamais l'inverse. Un claim prouvé seulement sur un mock ou un test unitaire ne peut pas passer `functional_proof: verified` quand une surface exécutable existe.
...
Le skill rend un bloc `verification` documenté dans [verify-output.md](references/verify-output.md) : `commit`, `ci`, `worktree`, `provenance`, `detected_surfaces`, `surface_gates`, `evaluator`, `carryover_gate`, `evidence_graph`, `matrix`, `readiness_usage`, `app_verification`, `functional_proof`, `approval_readiness`, `external_approval`, `causal_return`, `status`. Le coeur en est une `claim-by-claim evidence matrix` avec `claim`, `status`, `evidence`, `freshness`, `oracle` et `risk`.
