...
5. Fixer les conventions de connaissance : Atlas (vocabulaire commun) universel (`ship`, `PR`, `commit`, `worktree`, `QA`, `runtime`, `API`, preuve, oracle, rollback), emplacement du `CONTEXT.md` ou équivalent, emplacement des ADR et règles de décision, lexique métier, lexique technique et pont langage humain -> terme métier -> terme technique -> moyen de preuve.
6. Fixer les conventions d'architecture et de réussite : emplacement de la Boussole (cadrage : utilisateur, problème, succès, limites), System Success Map produit/technique/opérationnelle/commerciale, Project Decision Graph et Route Pack (pointeurs de contexte) de langage, architecture, recherche et preuve.
7. Fixer les conventions de preuve et readiness : emplacement des Proof Contracts, Readiness Forecasts et evidence bundles, distinction entre Functional Proof, Approval Readiness et External Approval, et règle d'invalidation/revalidation d'un artefact périmé ; si le dépôt a une surface exécutable, appeler `$verifier-application-azd generer` pour écrire le skill repo-local `verifier-<app>` et amorcer sa feature map avec `statut: never` pour chaque fonctionnalité connue.
8. Pour un projet `Standard` ou `Critical` seulement, si plusieurs inconnues indépendantes le justifient, constituer un Bootstrap Council (conseil d'amorçage) borné (architecture, risques, preuve, opportunités), dimensionné par scope utile, risque et indépendance, jamais par nombre de fichiers ; son absence ou indisponibilité ne bloque pas l'init.
