...
7. Fixer les conventions de preuve et readiness : emplacement des Proof Contracts, Readiness Forecasts et evidence bundles, distinction entre Functional Proof, Approval Readiness et External Approval, et règle d'invalidation/revalidation d'un artefact périmé ; amorcer la feature map (`skills/prouver-resultat-azd/references/feature-map-template.md`) avec `statut: never` pour chaque fonctionnalité connue.
